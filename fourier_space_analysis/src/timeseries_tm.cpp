...
int main(int argc,char *argv[])
{
    ...
    Array2D<fftw_complex> AllSq;
    Sq.resize(num_samples);
    AllSq.resize(nk,num_samples);

    std::ifstream datain(datfile.c_str());
    if(!datain.is_open())
    {
        std::cerr << "Error: Could not open file for reading time series" << std::endl;
        exit(0);
    }
    ...
    for(unsigned int t = 0 ; t < num_samples ; t++)
    {
        ...
        for(unsigned int c = 0 ; c < nk ; c++)
        {
            ...
            if(windowi==1)
            {
                ...
                AllSq(c,t)[1]*=(hammalpha-hammbeta*cos(2.0*M_PI*static_cast<double>(t)/(static_cast<double>(num_samples)-1.0)));
            }
        }
    }
    datain.close();
    ...
    for(unsigned int i = 0 ; i < nk ; i++)
    {
        ...
        ores.resize(num_samples);
        res.IFill(0);
        ores.IFill(0);
        for(int w = -num_samples/2 ; w < num_samples/2 ; w++)
        {
            int lures=w+(num_samples/2);
            int luHq=0;
            if(w<0){luHq=w+num_samples;}else{luHq=w;}
            //calculate the bits of the one-sided PSD
            const double Hq = Sq(luHq)[0]*Sq(luHq)[0] + Sq(luHq)[1]*Sq(luHq)[1];
            std::cout << Sq(luHq)[0] << "\t" << Sq(luHq)[1] << std::endl;
            res(lures)=Hq;
            ores(lures)=res(lures);
        }
        FIXOUT(Info,"Applying 1D Gaussian filter:" << std::flush);
        gaussianiir1d(ores,res,num_samples,pixelwidth,10);
        SUCCESS(Info);
        ...
        for(unsigned int w = -num_samples/2 ; w < num_samples/2 ; w++)
        {
            int lures=w+(num_samples/2);
            int luHq=0;
            if(res(lures)>maxsmooth && static_cast<double>(lures)*2*M_PI/(static_cast<double>(num_samples)*dt) > fcutl && static_cast<double>(lures)*2*M_PI/(static_cast<double>(num_samples)*dt) < fcutu)
            {
                maxsmooth=res(lures);
                maxsmoothindex=lures;
            }
            if(ores(lures)>maxraw && w>1 && static_cast<double>(lures)*2*M_PI/(static_cast<double>(num_samples)*dt) > fcutl && static_cast<double>(lures)*2*M_PI/(static_cast<double>(num_samples)*dt) < fcutu)
            {
                maxraw=ores(lures);
                maxrawindex=lures;
            }
...
//calculate the bits of the one-sided PSD
            dataout << freq << "\t" << ores(lures) << "\t" << res(lures) << "\t" << ores(lures)/maxraw << "\t" << res(lures)/maxsmooth << std::endl;
        }
        dataout.close();
        if(dataout.is_open())
        {

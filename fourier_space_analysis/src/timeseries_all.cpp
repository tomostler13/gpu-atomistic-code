...
//        std::cin.get();
        ...
        Array<double> res,ores,reres,imres;
        res.resize(num_samples/2);
        ores.resize(num_samples/2);
        reres.resize(num_samples/2);
        imres.resize(num_samples/2);
        res.IFill(0);
        ores.IFill(0);
        reres.IFill(0);
        imres.IFill(0);
...
//calculate the bits of the one-sided PSD
            dataout << freq << "\t" << reres(w) << "\t" << imres(w) << "\t"  << ores(w) << "\t" << res(w) << "\t" << ores(w)/maxraw << "\t" << res(w)/maxsmooth << std::endl;
        }
        dataout.close();
        if(dataout.is_open())
        {

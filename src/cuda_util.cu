...
// Author:Tom Ostler
// Created: 26/06/2014
// Last-modified: 06 Jun 2015 19:19:52
#include "../inc/cuda.h"
...
namespace cullg
{
    ...
    void allocate_memory_on_card()
    {
        ...
        if(exch::inc4spin)
        {
            CUDA_CALL(cudaMalloc((void**)&Cxadj_jkl,exch::xadj_j.size()*sizeof(int)));
            CUDA_CALL(cudaMalloc((void**)&Cadjncy_j,exch::adjncy_j.size()*sizeof(int)));
            CUDA_CALL(cudaMalloc((void**)&Cadjncy_k,exch::adjncy_k.size()*sizeof(int)));
            CUDA_CALL(cudaMalloc((void**)&Cadjncy_l,exch::adjncy_l.size()*sizeof(int)));
            CUDA_CALL(cudaMemcpy(Cxadj_jkl,exch::xadj_j.ptr(),exch::xadj_j.size()*size(int),cudaMemcpyHostToDevice));
            CUDA_CALL(cudaMemcpy(Cadjncy_j,exch::adjncy_j.ptr(),exch::adjncy_j.size()*size(int),cudaMemcpyHostToDevice));
            CUDA_CALL(cudaMemcpy(Cadjncy_k,exch::adjncy_k.ptr(),exch::adjncy_k.size()*size(int),cudaMemcpyHostToDevice));
            CUDA_CALL(cudaMemcpy(Cadjncy_l,exch::adjncy_l.ptr(),exch::adjncy_l.size()*size(int),cudaMemcpyHostToDevice));
        }

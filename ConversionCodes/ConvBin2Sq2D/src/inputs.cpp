...
// Author:Tom Ostler
// Created: 25 Feb 2016
// Last-modified: 22 Jun 2018 10:20:12
...
namespace inputs
{
    ...
    unsigned int dimin[3]={0,0,0},Nk[3]={0,0,0};
    ...
    void readcff(int argc,char *argv[])
    {
        ...
        libconfig::Setting &setting = cfg.lookup("bin2Sq2D");
        ...
        FIXOUTVEC(Info,"Size of unit cell (abc) [Angstroms]:",abc[0],abc[1],abc[2]);
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read the filename base (bin2Sq2D:InputFilenameBase (string))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read start time index (bin2Sq2D:StartTime (int))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read end time index (bin2Sq2D:EndTime (int))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read step in time index (bin2Sq2D:dTime (int))");
        }

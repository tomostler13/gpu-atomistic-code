...
// Last-modified: 22 Jun 2018 09:24:45
...
//containing the relative spin components as text
#include <cmath>
#include <iostream>
#include <libconfig.h++>
#include <fstream>
...
//        std::cin.get();
        ...
        std::ofstream ofs(ostr.c_str(),std::ios::out);
        if(!ofs.is_open())
        {
            error::errPreamble(__FILE__,__LINE__);
    ...
    std::ofstream posofs("pos.txt",std::ios::out);
    if(!posofs.is_open())
    {
        error::errPreamble(__FILE__,__LINE__);
    ...
    else
    {
        for(unsigned int i = 0 ; i < inputs::nspins ; i++)
        {
            posofs << posx[i] << "\t" << posy[i] << "\t" << posz[i] << std::endl;
        }
    }
    posofs.close();

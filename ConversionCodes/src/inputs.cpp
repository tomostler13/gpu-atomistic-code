...
// Author:Tom Ostler
// Created: 25 Feb 2016
// Last-modified: 25 Feb 2016 22:08:20
...
{
    //for outputting info
    std::ofstream Info("convVTU_info.dat");
    ...
    void readcff(int argc,char *argv[])
    {
        ...
        libconfig::Setting &setting = cfg.lookup("conv");
        ...
        FIXOUTVEC(Info,"Num grid points (Nk):",Nk[0],Nk[1],Nk[2]);
        for(unsigned int i = 0 ; i < 3 ; i++)
        {
            try
            {
                dimout[i] = setting["NCellsOut"][i];
            }
            catch(const libconfig::SettingNotFoundException &snf)
            {
                error::errPreamble(__FILE__,__LINE__);
        ...
        FIXOUTVEC(Info,"Dimensions out:",dimout[0],dimout[1],dimout[2]);
        for(unsigned int i = 0 ; i < 3 ; i++)
        {
            try
            {
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read the filename base (conv:InputFilenameBase (string))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read start time index (conv:StartTime (int))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read end time index (conv:EndTime (int))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read step in time index (conv:dTime (int))");
        }

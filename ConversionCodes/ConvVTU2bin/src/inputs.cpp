...
// Author:Tom Ostler
// Created: 22 Jun 2018
// Last-modified: 22 Jun 2018 09:10:51
...
{
    //for outputting info
    std::ofstream Info("convVTU2bin.info");
    ...
    void readcff(int argc,char *argv[])
    {
        ...
        libconfig::Setting &setting = cfg.lookup("convVTU2bin");
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read the filename base (convVTU2bin:InputFilenameBase (string))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read start time index (convVTU2bin:StartTime (int))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read end time index (convVTU2bin:EndTime (int))");
        }
        ...
        {
            error::errPreamble(__FILE__,__LINE__);
            error::errMessage("Could not read step in time index (convVTU2bin:dTime (int))");
        }

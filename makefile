...
GCC=g++ -std=c++11 -g $(OPTLEVEL)  -DCOMP='"GNU C++ Compiler $(shell g++ --version | head -n 1 | cut -b 5-)"' -DHOSTNAME='"$(shell hostname)"' ${GITINFO}
#add for debugging: -g -Wsign-compare -pedantic -Werror -Wfatal-errors 
NVCC=nvcc -std=c++11  $(OPTLEVEL) -DCOMP='"NVIDIA C++ Compiler $(shell nvcc --version | tail -n 2 | head -n 1)"' -DHOSTNAME='"$(shell hostname)"' ${GITINFO}
export LANG=C
export LC_ALL=C
# LIBS
DEFS=-DNDEBUG
